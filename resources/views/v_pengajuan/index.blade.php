...
<div class="card">
    ...
    <div class="card-body">
        <ul class="nav nav-tabs mb-4">
            <li class="nav-item">
                <a class="nav-link @if (request()->input('type') == 'pending' || request()->input('type') == null)
                active
                @endif" href="{{ route('pengajuan') }}">Pending</a>
            </li>
            <li class="nav-item">
                <a class="nav-link @if (request()->input('type') == 'setuju')
                    active
                    @endif" href="{{ route('pengajuan',['type'=> 'setuju']) }}">Setuju</a>
            </li>
            <li class="nav-item">
                <a class="nav-link @if (request()->input('type') == 'tolak')
                    active
                    @endif" href="{{ route('pengajuan',['type'=> 'tolak']) }}">Tolak</a>
            </li>
        </ul>
                ...
                <tr>
                    ...
                    <td class="text-md-center">
                        <form action="{{ route('update.pengajuan', $row->id) }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-primary btn-accept" data-id="{{ $row->id }}">Terima</button>
                        </form>
                        <form action="{{ route('update.pengajuan', $row->id) }}" method="POST">
                            @csrf
                            @method("PATCH")
                            <input type="hidden" name="status" value="tolak">
                            <button type="button" class="btn btn-danger btn-reject">Tolak</button>
                        </form>
                    </td>
                    @endif
                    @endcan
